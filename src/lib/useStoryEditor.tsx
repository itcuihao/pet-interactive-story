  ...
  const items = STEP_LABELS.map((item) => ({
    ...
    content:
      item.id === "story" ? (
        ...
        <PreviewStep draft={draft} onBack={goPrevStep} onPreview={onPreview} onSharePreview={onSharePreview} onShare={() => onShare(draft)} onExportJson={() => downloadStoryAsJson(draft)} onExportHtml={handleExportHtml} onDone={onDone} exportIssues={exportIssues} />
      ),
  }));
